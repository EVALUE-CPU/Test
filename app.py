...
def load_all_data():
    ...
    save    = load("Save.csv")
    return degree, car, station, count, save

def compute_total_score(phone: str, degree, car, station, count, save) -> int:
    total = 0
    for df, col in [
        (degree,  "TotalScore"),
        (car,     "CarTotalScore"),
...
def count_winners(degree, car, station, count, save) -> dict[int, int]:
    """回傳每個門檻有多少人達標"""
    all_phones = set()
    for df in [degree, car, station, count, save]:
        if not df.empty and "Phone" in df.columns:
            all_phones.update(df["Phone"].tolist())

    scores = {}
    for ph in all_phones:
        scores[ph] = compute_total_score(ph, degree, car, station, count, save)
...
# ── 載入資料 ──────────────────────────────────────────────
degree, car, station, count, save = load_all_data()
winners_map = count_winners(degree, car, station, count, save)
...
if query_btn and phone_input:
    ...
    total = compute_total_score(phone, degree, car, station, count, save)
